...
class pp_visualizer{
	...
	public:
		...
		void callback(const sensor_msgs::ImageConstPtr local_costmap, const nav_msgs::PathConstPtr globpath_nearby){
			...
			*/
				cv::circle(visualized_img, cv::Point(globpath_nearby->poses[i].pose.position.x, globpath_nearby->poses[i].pose.position.y), 2, cv::Scalar(255,255,255),-1);
				cv::arrowedLine(visualized_img, cv::Point(globpath_nearby->poses[i].pose.position.x, globpath_nearby->poses[i].pose.position.y), cv::Point(globpath_nearby->poses[i].pose.position.x + 5*cos(globpath_nearby->poses[i].pose.position.z*2*M_PI/180), globpath_nearby->poses[i].pose.position.y- 5*sin(globpath_nearby->poses[i].pose.position.z*2*M_PI/180)), 2, 4, 1, 2);
				//need to add arrows
			}
