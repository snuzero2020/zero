...
#define REF_X 298437
#define REF_Y 4137742

class Map_Marker{
    public:
    Map_Marker(){
        pub_gps =  n_.advertise<slam::Gps>("gps", 2);
    ...
    void callback_filtered(const slam::Data::ConstPtr& msg){
        ...
        ROS_DEBUG_STREAM("imu_marker: Kalman-filter: (" << msg->x << ", " << msg->y << ") -> position(" << pixel_x << ", " << pixel_y << \
    ...
    void callback_imu(const slam::Imu::ConstPtr& msg){
        ...
        ROS_DEBUG_STREAM("imu_marker: IMU: theta(" << msg->theta << ") -> position(" << pixel_x << ", " << pixel_y << ")");
        ...
        if (n % 10 == 0) {
            ...
            if( n % 200 == 0){
                //cv::arrowedLine(img, cv::Point(filtered_pixel_x, filtered_pixel_y), cv::Point(filtered_pixel_x+filtered_pixel_vx, filtered_pixel_y+filtered_pixel_vy), cv::Scalar(255, 0, 255), 10, 8, 0, 0.2);
                cv::arrowedLine(img, cv::Point(filtered_pixel_x, filtered_pixel_y), cv::Point(filtered_pixel_x+filtered_pixel_thx, filtered_pixel_y+filtered_pixel_thy), cv::Scalar(255, 255, 0), 10, 8, 0, 0.2);
                cv::arrowedLine(img, cv::Point(filtered_pixel_x, filtered_pixel_y), cv::Point(filtered_pixel_x+mag_pixel_thx, filtered_pixel_y+mag_pixel_thy), cv::Scalar(255, 0, 255), 10, 8, 0, 0.2);
            }
        }
