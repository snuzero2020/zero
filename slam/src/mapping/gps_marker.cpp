...
class Map_Marker{
    ...
    void callback_gps(const slam::Gps::ConstPtr& msg){
        ...
        if (n != 1) {
            ...
            if( n % 80 == 0){
                cv::arrowedLine(img, cv::Point(filtered_pixel_x, filtered_pixel_y), cv::Point(filtered_pixel_x+4*filtered_pixel_vx, filtered_pixel_y+4*filtered_pixel_vy), cv::Scalar(255, 0, 255), 16, 8, 0, 0.2);
                //cv::arrowedLine(img, cv::Point(filtered_pixel_x, filtered_pixel_y), cv::Point(filtered_pixel_x+10*filtered_pixel_thx, pixel_y+10*filtered_pixel_thy), cv::Scalar(255, 255, 0), 10, 8, 0, 0.5);
                cv::arrowedLine(img, cv::Point(pixel_x, pixel_y), cv::Point(pixel_x+4*mag_pixel_thx, pixel_y+4*mag_pixel_thy), cv::Scalar(255, 255, 0), 16, 8, 0, 0.2);
            }
        }
        t = ros::Time::now();
        saved = false;
